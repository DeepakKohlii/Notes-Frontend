...
const Archive = ({
  ...
  const unarchiveNote = (archiveNote: any) => {
    const updatedNotes = archivedNotes.filter(
      (data: any) => data.id !== archiveNote.id
    );
    setArchivedNotes(updatedNotes);
    setNotes((prevArr: any) => [...prevArr, archiveNote]);
  };

  const deleteNote = (archiveNote: any) => {
    const updatedNotes = archivedNotes.filter(
      (data: any) => data.id !== archiveNote.id
    );
    setArchivedNotes(updatedNotes);
    setDeletedNotes((prevArr: any) => [...prevArr, archiveNote]);
  };
